...
_load_library:
    ...
    NextClusterNumber   equ 0x52F0
    RootDirectoryEntry  equ 0x52F4
...
.data_loop:
    ...
    mov eax, dword [pLoadAddress]
    mov dword [DiskAddressPacket + 4], eax
...
.data_next_save:
    ...
    add ax, word [pLoadAddress]
    mov word [pLoadAddress], ax
    ;  다음 클러스터의 데이터를 위한 주소값 증가
    ...
    .read_disk:
        ...
        ; 만약 다음 인덱스가 0xFFFFFFFF
        jc .error_or_end

    jmp .data_loop
    ; 루프를 돌면서 지속적으로 다음 데이터 영역을 읽는다.
